...
type Gadget interface {
	...
	AddAlarm(action func(current string), when Object) //AddAlarm adds function that would be called when Gadget reaches in selected time. Parameter current in function is string-conversion of current time that inner function can uses.
